...
- `data/processed/asp_black_summer.qgz` — proyecto QGIS con las 11 capas precargadas y estilizadas (EPSG:3577), abre centrado en NSW (script `13`). Trae la reproyección al vuelo activada, así que admite mapas base (XYZ tiles en EPSG:3857) sin desalinear las capas. El bundle `asp-black-summer-capas-qgis.zip` (no versionado) empaqueta el proyecto + datos + estilos para enviarlo y abrirlo en Windows sin configurar nada.
